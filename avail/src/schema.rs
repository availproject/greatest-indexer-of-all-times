...
pub trait BasicTableOperations<T> {
		...
		T: for<'r> FromRow<'r, PgRow> + Tabled + Send + Unpin,
	{
		...
		let table = tabled::Table::new(result);
		println!("{}", table);
		Ok(())
	}
}
...
impl CarsEntry {
	pub fn new(brand: &str, model: &str, year: i32) -> Self {
		Self { brand: brand.into(), model: model.into(), year }
	}

	pub async fn table_insert_entry(&self, conn: &mut PgConnection) {
		if !Self::table_exists(conn).await.unwrap() {
			panic!("Table cars does not exist, dummy");
		}

		let q = "INSERT INTO cars (brand, model, year) VALUES ($1, $2, $3)";
		sqlx::query(q)
			.bind(self.brand.as_str())
	...
	pub async fn table_create(conn: &mut PgConnection) {
		if Self::table_exists(conn).await.unwrap() {
			panic!("Table cars already exist, dummy");
		}
	...
	"#;
		...
		sqlx::query(q).execute(conn).await.unwrap();
	}
}

impl BasicTableOperations<Self> for CarsEntry {
	const TABLE_NAME: &str = "cars";
}

impl<'r> FromRow<'r, PgRow> for CarsEntry {
	fn from_row(row: &'r PgRow) -> Result<Self, sqlx::Error> {
		Ok(Self {
